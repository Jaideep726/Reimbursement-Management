      ...
      <div className="w-full max-w-sm rounded-2xl p-6 shadow-2xl bg-white">
        ...
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            disabled={loading}
            className="flex-1 py-2.5 rounded-lg text-sm font-medium text-gray-700 border border-gray-200 hover:bg-gray-50 transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-white bg-red-600 hover:bg-red-700 disabled:bg-red-300 transition-colors"
          >
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6">
          ...
          {activeTab === 'All Expenses'   && <AllExpensesTab   companyId={profile?.company_id} />}
        </div>
      </div>
    </div>
  )
    ...
    <div>
      ...
      {!showForm && (
        loading ? (
          ...
          <p className="text-sm text-gray-400 py-8 text-center">No rules yet. Create one to get started.</p>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-2 flex-wrap">
                      {rule.sequential && (
                        <span className="px-2 py-0.5 rounded-full text-xs bg-purple-100 text-purple-700 border border-purple-200">Sequential</span>
                      )}
                      {rule.is_manager_approver && (
                        <span className="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 border border-blue-200">Manager first</span>
                      )}
                      {rule.min_approval_pct > 0 && (
                        <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-100 text-yellow-700 border border-yellow-200">{rule.min_approval_pct}% required</span>
                      )}
                    </div>
                  </div>
...
function CreateRuleForm({ companyId, managers, editingRule, onSaved, onCancel }) {
  ...
  const handleAddApprover = () => {
    ...
    setApprovers(prev => [...prev, { id: manager.id, name: manager.name, isRequired: false }])
    setSelectedManager('')
  }
  ...
  const handleToggleRequired = (id) =>
    setApprovers(prev => prev.map(a => a.id === id ? { ...a, isRequired: !a.isRequired } : a))
  ...
  const inputClass = (field) =>
    `w-full px-4 py-2.5 rounded-lg text-sm border focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors
     ${fieldErrors[field] ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <input type="text" value={ruleName} onChange={e => setRuleName(e.target.value)}
          placeholder="e.g. Travel Expense Rule" className={inputClass('ruleName')} />
      ...
      <div>
        ...
        <input type="text" value={description} onChange={e => setDescription(e.target.value)}
          placeholder="e.g. For all travel expenses above ₹5000" className={inputClass('description')} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-slate-50">
          ...
          <button type="button" onClick={() => setIsManagerApprover(v => !v)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isManagerApprover ? 'bg-blue-600' : 'bg-gray-200'}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${isManagerApprover ? 'translate-x-6' : 'translate-x-1'}`} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-slate-50">
          ...
          <button type="button" onClick={() => setIsSequential(v => !v)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isSequential ? 'bg-blue-600' : 'bg-gray-200'}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${isSequential ? 'translate-x-6' : 'translate-x-1'}`} />
          </button>
        </div>
      </div>
      ...
      <div>
        <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
          Minimum Approval % <span className="normal-case font-normal text-gray-400 ml-1">(0 = no approval needed)</span>
        </label>
        <input
          type="text"
          inputMode="numeric"
      ...
      <div>
        ...
        <div className="flex gap-2 mb-3">
          <select value={selectedManager} onChange={e => setSelectedManager(e.target.value)}
            className="flex-1 px-4 py-2.5 rounded-lg text-sm border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="">— Select a manager to add —</option>
            {managers.filter(m => !approvers.find(a => a.id === m.id)).map(m => (
              <option key={m.id} value={m.id}>{m.name}</option>
            ))}
          </select>
          <button type="button" onClick={handleAddApprover} disabled={!selectedManager}
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-medium rounded-lg transition-colors">
            Add
          </button>
        </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button onClick={handleSave} disabled={loading}
          className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed transition-colors">
...
function UsersTab({ companyId }) {
  ...
  const handleUserSaved = () => { setShowModal(false); setEditingUser(null); fetchUsers() }
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3">
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${ROLE_BADGE[user.role] ?? ROLE_BADGE.employee}`}>
          {user.role}
        </span>
      </td>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1">
          <button onClick={onEdit}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-blue-600 border border-blue-200 hover:bg-blue-50 transition-colors">
            ✏️ Edit
          </button>
          <button onClick={onDelete}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-red-500 border border-red-200 hover:bg-red-50 transition-colors">
            🗑️ Delete
          </button>
        </div>
      </td>
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        const { error: updateErr } = await supabase
          .from('users')
          .update({ name, role, manager_id: role === 'employee' ? managerId : null })
    ...
    return (
      <Overlay onClose={onSaved}>
        <div className="text-center">
          ...
          <div className="bg-gray-50 border border-dashed border-gray-300 rounded-xl px-6 py-4 mb-6 font-mono text-xl font-bold text-gray-800 tracking-widest select-all">
            {tempPassword}
          </div>
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <p className="text-sm text-gray-500 mb-5">
        {isEditing ? 'Update the details below. Email cannot be changed here.' : 'A temporary password will be generated automatically.'}
      </p>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {isEditing ? (
            <div className="w-full px-4 py-2.5 rounded-lg text-sm border border-slate-200 bg-slate-100 text-gray-400 cursor-not-allowed">
              {email}
            </div>
          ) : (
            <>
...
function AllExpensesTab({ companyId }) {
  ...
  async function fetchExpenses() {
    ...
    try {
      const { data, error } = await supabase
        .from('expenses')
        .select('*, users!inner(name, company_id)')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={fetchExpenses} className="text-xs text-blue-600 hover:underline">↻ Refresh</button>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-400 py-8 text-center">No expenses submitted yet.</p>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-gray-100">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
            ...
            <tbody className="divide-y divide-gray-100">
              {expenses.map(exp => (
                <tr key={exp.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-gray-900">{exp.users?.name || '—'}</td>
                  <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{fmtDate(exp.date || exp.created_at)}</td>
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                    ...
                    {exp.converted_amount != null && exp.company_currency && exp.company_currency !== exp.currency && (
                      <span className="ml-1 text-xs text-gray-400">
                        ({exp.company_currency} {Number(exp.converted_amount).toFixed(2)})
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[exp.status] || STATUS_STYLES.draft}`}>
                      {exp.status || 'draft'}
                    </span>
                  </td>
                </tr>
